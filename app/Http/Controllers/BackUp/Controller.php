...
class Controller extends BaseController
{
    ...
    public function SendNotifFCM($title, $isi){
        // var_dump($nama);

        $GetDataFCM = DB::table('data_fcm')
            ->select('data_fcm.server_key','data_fcm.registeration_id')
            ->first();
        $GetDataToken = DB::table('user_token')
                        ->where('token', '<>','-')
                        ->pluck('token');
        
        $listval=array();
        $listval =  $GetDataToken;

            $url = "fcm.googleapis.com/fcm/send";
            // $token = $GetDataFCM->registeration_id;
            // $token = json_encode($GetDataToken);
            $serverKey = $GetDataFCM->server_key;
            $title = $title;
            $body = $isi;
            $notification = array('title' =>$title , 'body' => $body, 'sound' => 'default', 'badge' => '1');
            // $arrayToSend = array('to' => $token, 'notification' => $notification,'priority'=>'high');
            $arrayToSend = array('registration_ids' => $listval, 'notification' => $notification,'priority'=>'high');
